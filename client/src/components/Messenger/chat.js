...
import {
  conversationsOfUsersStart,
  getRepliesOfConversationStart,
  addMsgInReplies,
...
const Messenger = () => {
  ...
  useEffect(() => {
    ...
    socket.emit("chat-connect", { userId: authUser?._id });
  ...
  useEffect(() => {
    if (currentChatUser)
      dispatch(
        getRepliesOfConversationStart({ conversationId: currentConversationId })
      );
  }, [currentChatUser]);

  useEffect(() => {
    console.log("running");
    messagesInputRef.current?.scrollIntoView({ behavior: "smooth" });
  ...
  const renderConversation = (conversation) => {
    const user =
      authUser?._id === conversation.userOneId._id
  ...
  const renderConversationReply = (reply) => {
    const isLoggedInUser = reply.userId._id === authUser?._id;
                ...
                <div className="chatapp_list">
                  ...
                  <ul className="right_chat list-unstyled mb-0">
                    {conversation.conversationData?.map((conv) => {
                      return conv && renderConversation(conv);
                    })}
                  </ul>
                </div>

                <div
                  className="chatapp_body mr-0 d-flex flex-column"
                  style={{ height: "75vh" }}
                >
                    ...
                    <div className="row clearfix">
                      <div className="col-lg-12">
                        <div className="chat-about">
                          {currentChatUser && (
                            <h6 className="m-b-0">{`${currentChatUser?.firstName} ${currentChatUser?.lastName}`}</h6>
                          )}
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="chat-history flex-grow-1 overflow-auto">
                    <ul className="message_data">
                      {conversation.replies?.map(
                        (reply) => reply && renderConversationReply(reply)
                      )}
                    </ul>
                    <div ref={messagesInputRef} />
                  </div>
